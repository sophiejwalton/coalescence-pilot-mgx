    ...
    for inoculumn in inoculumn_list:
       ...
       # parent_samples = np.intersect1d(parent_samples, freq_filtered.columns.values)
        ...
        for fname in glob(f'{save_dir}/*/{inoculumn}_parent_freqs.csv'):
            ...
            df_infogr = df_info.loc[~df_info['is_inoculumn'],:].groupby(['mesocosm']).sum()
        ...
        full_df.to_csv(f'{save_dir}/{inoculumn}_winners.csv')
