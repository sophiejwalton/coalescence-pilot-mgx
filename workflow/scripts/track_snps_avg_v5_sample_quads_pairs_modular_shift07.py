...
#        print(bs_sample1)
            int11 = round(len(bs_sample1)/2)
            bssample11 = bs_sample1[:int11]

            #int21 = round(2*len(bs_sample1)/2)
            bssample21 = bs_sample1[int11+1:]
 

            int12 = round(len(bs_sample2)/2)
            bssample12 = bs_sample2[:int12]
           # int22 = round(2*len(bs_sample2)/2)
            bssample22 = bs_sample2[int12+1:]
...
#            passage4_est=get_sample_freq_adjust(freq_children,reads_children, reads_children_opp, sample4, bssample41,bssample42, depth_med)
    ...
    df = pd.DataFrame(data = { 'mesocosms': mesocosms, 
