...
def get_bootstrap_sel_coeffs(metadata, freq_children, depth_med, parent_snps, n_bootstraps = 10000, initial_sample = 1, final_sample=3,):
   ...
   # print(snps.columns.values)
    ...
    for mesocosm in metadata['mesocosm'].unique():
        ...
        snps_meso = freq_children.loc[parent_snps, meso_df['sample'].unique()]
        how_many_good = snps_meso.count(axis=0) == len(meso_df['sample'].unique())
        snps_meso_good = how_many_good[how_many_good].index.values
        n_snps = len(n_snps)
        
        if 0 in passages:
            passages.remove(0)
        early_passages = passages[passages<4]
        late_passages = passages[passages>=4]

        if len(early_passages) < 2:
            continue
        if len(late_passages) < 1:
            continue 
...
#        if initial_sample == 'both' and (inoculumn not in metadata['sample'].values):
 #           continue 
        ...
        diffs_late = np.zeros((n_bootstraps*len(late_passages)))
        passages_looked_at = np.zeros((n_bootstraps*len(late_passages)))
        sel_coeffs = np.zeros(n_bootstraps*len(late_passages))

        for sample in range(n_bootstraps):
            bs_sample = np.random.choice(parent_snps_good, size=len(parent_snps),replace=False)
            ...
            passage7_est=np.nan
            if 5 in passages:
                passage5_est1 = get_freq_est(snps1.loc[bs_sample1[int21+1:int31],sample5].values, 
                        depth_med[sample5].values[0] )
                passage5_est2 = get_freq_est(snps2.loc[bs_sample2[int22+1:int32],sample5].values, 
                        depth_med[sample5].values[0] )
                passage5_est=passage5_est1/(passage5_est1+passage5_est2)
            if 7 in passages:
