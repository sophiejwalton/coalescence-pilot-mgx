...
def get_main(subjects, species_dir, save_dir, species):
    ...
    for i, subject in enumerate(subjects):
        ...
        for t1,t2 in it.combinations(focal_times, 2): 
            ...
            if len(freq_transition_filter) == 0: 
                ...
                freq_before = freq_transition_filter[[s1]].median().values[0]
                median_freq_before_switch.append(freq_before)
                print(float(freq_before))
                percent_zero = np.sum(freq_transition_filter[[s1]] == 0.).values[0]/len(freq_transition_filter)
                percent_snps_at_zero.append(percent_zero)


        ss_df = pd.DataFrame(data = {'t1': t1s, 't2': t2s, 'snps_switching': snps_switch, 'median_freq': median_freq_before_switch, 'percent_snps_at_zero': percent_snps_at_zero })
    ...
    all_strain_shift_dfs.to_csv(f'{save_dir}/all_comparisons_frequencies_before_shift.csv')
