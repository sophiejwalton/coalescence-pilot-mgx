...
def get_main(subjects, save_dir, species, main_study):
    ...
    for i, subject in enumerate(subjects):
        ...
        good_samples_full = good_freq.columns.values
        good_times_all = np.array([int(sample.split('-')[-1]) for sample in good_samples_full])
    ...
    #    focal_times = np.array([int(sample.split('-')[-1]) for sample in focal_samples])
        for t1,t2 in it.combinations(good_times, 2): 
        ...
        ss_df['Prexisting'] = ss_df['median_freq'].transform(get_preexisting)
        print(ss_df)
        all_strain_shift_dfs.append(ss_df)

    if len(all_strain_shift_dfs) == 0:
        all_strain_shift_dfs = pd.DataFrame(data = {'t1':[], 't2': [], 'snps_switching': [], 'median_freq': [], 'Subject': [],'Species': [], 'Strain Shift': [],
                                     'percent_snps_at_zero': [], 'Prexisting': []})
    else:
        all_strain_shift_dfs = pd.concat(all_strain_shift_dfs)
    if main_study:
        all_strain_shift_dfs.to_csv(f'{save_dir}/get_strain_shifts_main_study_timepoints.csv')
    else:
        all_strain_shift_dfs.to_csv(f'{save_dir}/get_strain_shifts_full_study_timepoints.csv')
...
if __name__ == '__main__':
    ...
    for subject in subjects:
        if subject == 'XHC':
            abx_subjects.append(subject)
        elif subject[0] == 'X' and subject[-1] == 'A':
            abx_subjects.append(subject)
  
    get_main(subjects, save_dir, args.species, args.main_study)
