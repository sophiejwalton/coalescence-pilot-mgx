...
rule mergeSpeciesp3:
    ...
    output:
        "workflow/out/midas2v3_output/merge/species/species_prevalence.tsv"
    params:
        midasdb=config['midasdb'],
        midasdb_dir=config['midasdb_dir']
    threads: config['maxCPUs']
    ...
    shell:
        """
        midas2 merge_species --samples_list workflow/out/list_of_samples.tsv --min_cov 1  midas2v3_output/merge
        """
...
def get_abundance_species():
    #pass
    df= pd.read_csv('workflow/out/midas2v3_output/merge/species/abundant_species.csv')
...
rule identifySNVs:
    ...
    shell:
        ...
        midas2 run_snps --sample_name {wildcards.sample} -1 {input.r1} -2 {input.r2}  --midasdb_name {params.midasdb} \
            --midasdb_dir {params.midasdb_dir} --snp_maf .001 --site_depth 1 --num_cores {threads} workflow/out/midas2v3_output  \
            --advanced \
...
#    df = pd.read_csv('workflow/out/midas2v3_output/species/species_prevalence.tsv',delimiter = '\t')
 #   df = df.sort_values(by = 'mean_coverage',ascending = False).reset_index()
    pass#','.join(df.loc[1:20,'species_id'].astype(str).to_list())
