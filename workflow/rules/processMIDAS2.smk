...
rule lz4_to_gzip:
    input:
        snpsDepth="workflow/out/midas2_output/merge/snps/{species}/{species}.snps_depth.tsv.lz4",
        snpsFreq="workflow/out/midas2_output/merge/snps/{species}/{species}.snps_freqs.tsv.lz4",
        snpsInfo="workflow/out/midas2_output/merge/snps/{species}/{species}.snps_info.tsv.lz4",
    output:
    ...
    shell:
        """
        lz4 -d {input.snpsDepth}
        gzip {output.snpsDepth}
        lz4 -d  {input.snpsFreq}
        gzip {output.snpsFreq}
        lz4 -d {input.snpsInfo}
