...
def get_bootstrap_parent(freq_children, depth_med, parent_snps, n_bootstraps = 1000):
    ...
    act_med = []
    samples_good = []
    for sample in snps.columns.values[:10]:
...
def get_main(parent_samples, child_samples, freq_filtered, depth_filtered):
    ...
    freq_inoculumns.to_csv('workflow/out/midas2_output/merge_100196/freq_inoculumns.csv.gz',compression='gzip')
    print(parent_samples)
    
    if len(freq_inoculumns.columns.values)<1:
    ...
    # is the marker 
    parent1_snps = get_parent_snps(freq_inoculumns, thresh = .8)
   # print(len(distinguishing_snps))
    parent1_snps.to_csv('parent1_snps.csv')
   
    print('before', len(parent1_snps))
    parent1_snps = filter_distinguishing_snps(freq_filtered[child_samples], parent1_snps, thresh = .5, sample_thresh=.75)
    print('after', len(parent1_snps))
    med_depth_children = depth_filtered.copy().replace(0, np.nan).median(skipna=True)

    count_parent1 = pd.DataFrame(get_count(freq_filtered[child_samples], parent1_snps)).rename(columns = {0: parent_samples[0]})  

    parent1_info = get_bootstrap_parent(freq_filtered[child_samples], med_depth_children, parent1_snps, n_bootstraps = 1000)
# freq_parent2 = freq_parent2.T
   # freq_parent2['parent'] = parent_samples[1]
   # print(freq_parent2)
    return count_parent1, parent1_info
...
#   freq_filtered_in.to_csv(f'{args.indir}/{args.species}/freq_filtered.csv.gz',compression='gzip')

    for genome in metadata['test_genome'].unique():
#        print(inoculumn)
        print(genome)
        all_samples = metadata.loc[metadata['test_genome'] == genome,'trial_parts'].values
        parent_samples = metadata.loc[(metadata['test_genome'] == genome)*(metadata['is_close_to_ref']),:]
        parent_samples = parent_samples.loc[parent_samples['trial'].astype(int) == 0,'trial_parts']
        ...
        count_parent1, frequency_estimates = get_main(parent_samples, all_samples, 
            freq_filtered_in[all_samples],  depth_filtered_in[all_samples])
        

       # count_parents.to_csv(f'{save_dir}/{inoculumn}_count_parents.csv')
        frequency_estimates.to_csv(f'{save_dir}/{genome}_parent1_info.csv')
