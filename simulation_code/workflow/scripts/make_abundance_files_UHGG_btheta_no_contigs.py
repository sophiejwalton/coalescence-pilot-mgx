...
def get_contig_abundance_file(abundance_df, contig_df):
   ...
   # print('SKX01058' in contig_df['species_id'].unique(), 'woow')
    ...
    contig_df_small['genome_rel_abundance'] = contig_df_small['rep_species'].transform(lambda x: abundance_df_small.loc[abundance_df_small['rep_species'] == x, 'relative_abundance'].values[0])
...
#    parser.add_argument('--genomedir', action='store',
 #                   help='where genomes stored at')
  ...
  #  genome_info = pd.read_csv(f'{args.genomedir}/genomes_info.csv').rename(columns={'species_id':'rep_species'})
    ...
    theta_genomes = ['GUT_GENOME001553','GUT_GENOME034180','GUT_GENOME001329','GUT_GENOME172048','GUT_GENOME000472',
...
#    theta_genomes=['GUT_GENOME034180','GUT_GENOME001329']#'GUT_GENOME000472','GUT_GENOME001553',
    ...
    # old - [0,.0001, .001,.01,.1,.2,.4,.5,.6,.8,.9,.99,.999,.9999, 1., .002,.005,.007,.992,.995,.997,.02,.05,.07,.98,.95,.93,.7]
    for theta_genome in theta_genomes:
        for j, theta_abundance_sim in enumerate([4/9, 5/9, 6/9, 1, 10/9, 5*(10/9),1/9,2/9,3/9]):
          for i,genome_abundance in enumerate([0,.00001, .0001, .001,.01,.1,.5, .9,.99,.999,.9999,.99999,1.]):
              outdir = f'{args.outdir}/{theta_genome}'
              if not path.isdir(outdir):
                  mkdir(outdir )
              #print(theta_genome, genome_abundance)
              speciesAbundances_small = speciesAbundances.copy()
              theta_abundance = theta_abundance_sim*(speciesAbundances_small.loc[speciesAbundances_small['rep_species'] == 'GUT_GENOME001120', 'relative_abundance'].values[0])
              ...
              other_sp = speciesAbundances_small.loc[speciesAbundances_small['rep_species']!='GUT_GENOME001120','rep_species'].values
             # print(genome_abundance)
            ...
            # df_Bv_small['relative_abundance']
              speciesAbundances_small = speciesAbundances_small._append(df_Bv_small)
              speciesAbundances_small.loc[speciesAbundances_small['rep_species'].isin(other_sp), 'relative_abundance'] =  speciesAbundances_small.loc[speciesAbundances_small['rep_species'].isin(other_sp), 'relative_abundance']/(1-theta_abundance)

              speciesAbundances_small['relative_abundance'] = speciesAbundances_small['relative_abundance']/(speciesAbundances_small['relative_abundance'].sum())
              print('theta_sample',speciesAbundances_small.loc[speciesAbundances_small['rep_species'] == theta_genome, 'relative_abundance'])
              print('party',speciesAbundances_small.loc[speciesAbundances_small['rep_species'] == 'GUT_GENOME001120', 'relative_abundance'])
              speciesAbundances_small.to_csv(f'{outdir}/{species_info_prev}_{theta_genome}_{str(i)}_abundances.csv')
              speciesAbundances_small[['rep_species','relative_abundance']].to_csv(f'{outdir}/{species_info_prev}_{theta_genome}_strain_{str(i)}_theta_{str(j)}_abundances.txt', sep='\t',
