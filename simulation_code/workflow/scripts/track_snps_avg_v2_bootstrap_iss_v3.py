...
def get_abuns(genome,sample_input,sample_infer, depth_theta_input, freqs, depths):

    # {species_info_prev}_{theta_genome}_strain_{str(i)}_theta_{str(j)}_abundances.txt'
    # GUT_GENOME000472_G8-e003Coalescence-mBHI-p7_S125_annotated_abundance_GUT_GENOME000472_strain_9_theta_5_abundances_trial_000
    p1 = f'{genome}_{sample_input}_annotated_abundance_{genome}_strain_11_theta_{depth_theta_input}_abundances_trial_000'
    p2 = f'{genome}_{sample_input}_annotated_abundance_{genome}_strain_0_theta_{depth_theta_input}_abundances_trial_000'
   # print(freqs.columns.values)
    if p1 not in freqs.columns.values:
        return []
    if p2 not in freqs.columns.values:
        return []
    ...
    inputs = []
    for i,abun in enumerate([0,.00001, .0001, .001,.01,.1,.5, .9,.99,.999,.9999,.99999,1]):
        for j, depth_theta in enumerate([4/9, 5/9, 6/9, 1, 10/9, 5*(10/9)]):
            for k in [0,1]:
                print(j, depth_theta,'woooooooo')
                sample = f'{genome}_{sample_input}_annotated_abundance_{genome}_strain_{str(i)}_theta_{str(j)}_abundances_trial_00{str(k)}'
                ...
                reads1 = np.nan
                reads0=np.nan
                depth_thetas.append(depth_theta)
                trials.append(k)
                if est ==0:
                    b = np.sum(reads_vals==1)/np.sum(reads_vals==0)
                    est_01 = b/depth_est_med
                    est=est_01
                    reads1 = np.sum(reads_vals==1)
                    reads0 = np.sum(reads_vals==0)
                if est == 1:
                    b = np.sum(reads_opp_vals==1)/np.sum(reads_opp_vals==0)
                    est_01 = 1-b/depth_est_med
                    est = est_01
                    reads1 = np.sum(reads_opp_vals==1)
                    reads0 = np.sum(reads_opp_vals==0)
                print(abun,est)
                estimated_abun_full.append(est)
                estimated_abun_01.append(est_01)
                estimated_abun_meds.append(est_med)
                reads_vals1.append(reads1)
                sample_infer_full.append(sample)
                print(sample,j,depth_theta)
                reads_vals0.append(reads0)
                inputs.append(abun)
    df = pd.DataFrame(data={'est':estimated_abun_full, 'input':inputs, 'est_meds': estimated_abun_meds, 'est_01': estimated_abun_01, 
                                'reads_vals1': reads_vals1, 'reads_vals0': reads_vals0, 'depth_thetas': depth_thetas,'trials':trials,
                                'sample_infer_full': sample_infer_full})
    df['genome']=genome
    #df['depth_theta_input'] = depth_theta_input
    df['sample_input']=sample_input
    df['sample_infer']=sample_infer
    df['sample_input_full1'] = p1
    df['sample_input_full2'] = p2
    return df 
...
#    parent_samples, child_samples = get_parent_children(args.inoculumn)
    ...
    good_samples = med_nonzero_depth[med_nonzero_depth>=0.]
    ...
    theta_depths = [4/9, 5/9, 6/9, 1, 10/9, 5*(10/9)]
    dfs = []
    for sisf in [('G8-e003Coalescence-mBHI-p7_S125','A10-e003Coalescence-mBHI-p7'),
        ...
        for genome in genomes:
            for j,theta_depth in enumerate(theta_depths):
                dfsm = get_abuns(genome,sample_input,sample_infer,j, freq_filtered_in, depth_filtered_in)
                if len(dfsm)>0: 
                    print('woo', j, theta_depth)
                    dfsm['theta_depth_input'] = theta_depth
                    dfsm.to_csv(f'{save_dir}/theta_depth_{str(j)}_sims_freqs.csv')
                    dfs.append(dfsm)
    df = pd.concat(dfs)
