...
def get_main(parent_samples, child_samples, freq_filtered, depth_filtered):
  #  info, depth, freq = load_and_sort_files(species_dir, species)
   ...
   # print(len(distinguishing_snps))
    ...
    parent1_info = get_bootstrap_parent(freq_children, med_depth_children, parent1_snps, n_bootstraps = 10000)
    parent2_info = get_bootstrap_parent(freq_children, med_depth_children, parent2_snps, n_bootstraps = 10000)
...
#    parent_samples, child_samples = get_parent_children(args.inoculumn)
 ...
 #   freq = repolarize_against_reference(freq, info)
    ...
    depth_filtered_in.to_csv(f'{species_dir}/depth_filtered_in.csv.gz',compression='gzip')
    freq_filtered_in.to_csv(f'{species_dir}/freq_filtered_in.csv.gz',compression='gzip')
    #metadata = pd.read_csv('workflow/analysis/e003_metadata_cultures_round2_change_AA.csv')
    metadata = pd.read_csv('metadata_sims.csv')
    metadata=metadata.loc[metadata['initial_sample'] == 'A10-e003Coalescence-mBHI-p7',:]
    #print(freq_filtered_in.min(axis=1))
    #print(freq_filtered_in.max(axis=1))
 #   print(len(parent_snps))
    
    for genome in metadata['test_genome'].unique():
#        print(inoculumn)
        metadata_genome = metadata.loc[metadata['test_genome'] == genome,:]
        all_samples = metadata.loc[metadata['test_genome'] == genome,'trial_parts'].values
        parent_sample1 = metadata_genome.loc[(metadata_genome['abundance_id'] == 12)*(metadata_genome['trial'].astype(int)==0),
                'trial_parts'].values[0]
        parent_sample2 = metadata_genome.loc[(metadata_genome['abundance_id'] == 0)*(metadata_genome['trial'].astype(int)==0),
                'trial_parts'].values[0]
        parent_samples = [parent_sample1,parent_sample2]
        parent_samples = list(np.intersect1d(parent_samples, freq_filtered_in.columns.values))
        all_samples = list(np.intersect1d(all_samples, freq_filtered_in.columns.values))
        if parent_sample1 in all_samples:
            all_samples.remove(parent_sample1)
        if parent_sample2 in all_samples:
            all_samples.remove(parent_sample2)
        #child_samples = freq_filtered_in.columns.values
        if len(parent_samples) < 2:
            continue
        count_parents, parent1_info, parent2_info = get_main(parent_samples, all_samples, 
            freq_filtered_in[parent_samples+all_samples],  depth_filtered_in[parent_samples+all_samples])

        #inoculumn = ''.join(inoculumn.split('/'))
        

        count_parents.to_csv(f'{save_dir}/{genome}_count_parents.csv')
        parent1_info.to_csv(f'{save_dir}/{genome}_parent1_info.csv')
        parent2_info.to_csv(f'{save_dir}/{genome}_parent2_info.csv')
