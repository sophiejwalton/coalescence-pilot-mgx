...
def build_genome_db(outdir, genomedir, species_list):
    ...
    for sp in species_list:
        ...
        for r in Bio.SeqIO.parse(infile, 'fasta'):
            long_contig = long_contig + str(r.seq).upper() + 'N'*300
        ...
        sp_name = sp.split('/')[-1][:-len('.gff.gz')]
        outfile.write('>%s\n%s\n' % (sp_name, str(long_contig)))
        species_names.append(sp_name)
        contig_lengths.append(len(long_contig))
        contig_names.append(sp_name)
        db_stats['total_seqs'] += 1
        db_stats['total_length'] += len(long_contig)
        infile.close()
    outfile.close()
...
if __name__ == '__main__':
   ...
   # parser.add_argument('--genomedir', action='store',
    ...
    species_list = glob.glob(f'{genome_dir}/*.gff.gz')
 #   genome_dir = 
    #species_list = [i.split('/')[-1][:-len('.gff.gz')] for i in species_list]
    args = parser.parse_args()
    if not path.isdir(args.outdir):
        mkdir(args.outdir)

    db_info = build_genome_db(args.outdir, genome_dir, species_list)
    db_info.to_csv(f'{args.outdir}/genomes_info.csv',index=False)
